...
import { batteryWidget, colonLayout, dateLayout, dayLayout, ring1Icon, ring1Layout, ring1Text, ring2Icon, ring2Layout, ring2Text, ring3Icon, ring3Layout, ring3Text, smallWidget1Icon, smallWidget1Text, smallWidget2Icon, smallWidget2Text, timeLayout } from "./index.r.layout";
...
WatchFace({
  ...
  buildBattery() {
    const levelWidget = hmUI.createWidget(hmUI.widget.IMG_LEVEL, batteryWidget);
  },
  ...
  buildRing3() {
    const progressWidget = hmUI.createWidget(hmUI.widget.ARC_PROGRESS, ring3Layout);

    hmUI.createWidget(hmUI.widget.IMG, ring3Icon);

    const textWidget = hmUI.createWidget(hmUI.widget.TEXT, ring3Text);


    let prevValue = null;

    const update = () => {
      const { current, target } = hmSensor.createSensor(hmSensor.id.STAND);

      if (prevValue === current) {
        return;
      }

      console.log('stand rerendered');
